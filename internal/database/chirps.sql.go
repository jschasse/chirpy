...
const chirpAuthorCheck = `-- name: ChirpAuthorCheck :one
SELECT id, created_at, updated_at, body, user_id
FROM chirps
WHERE id = $1 AND user_id = $2
`

type ChirpAuthorCheckParams struct {
	ID     uuid.UUID
	UserID uuid.UUID
}

func (q *Queries) ChirpAuthorCheck(ctx context.Context, arg ChirpAuthorCheckParams) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, chirpAuthorCheck, arg.ID, arg.UserID)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
